...
def hojo():
  ...
  <div class="box" style="margin-top:2.5mm;padding:2.5mm 4.5mm;line-height:1.6;">
    ...
    # 関東版：窓口と入口
    kanto = [
        ("東京都", "都の制度が大きい。断熱窓・太陽光・給湯器などの住宅系、区市町村の上乗せも多い",
         "クール・ネット東京／東京都環境局"),
        ("神奈川県", "県と市の両方に住宅・省エネ系。政令市（横浜・川崎・相模原）は独自制度が厚い", "県くらし安全防災局・各市"),
        ("埼玉県", "住宅リフォーム・耐震・空き家の制度が市町村単位で多い", "県住宅課・各市町村"),
        ("千葉県", "耐震・ブロック塀・浄化槽まわりの制度が目立つ", "県住宅課・各市町村"),
        ("茨城県", "移住定住・空き家の支援が比較的手厚い市町村がある", "県・各市町村"),
        ("栃木県", "住宅取得・三世代同居の支援がある市町がある", "県・各市町村"),
        ("群馬県", "住宅リフォーム・移住支援の制度がある市町村がある", "県・各市町村"),
    ]
    krows = "".join(
        f'<tr><td style="width:26mm;height:11mm;font-size:10pt;text-align:center;">'
        f'<b>{a}</b></td><td style="font-size:9pt;">{b}</td>'
        f'<td style="width:44mm;font-size:8.8pt;">{c}</td></tr>' for a, b, c in kanto)

    p4 = f'''<h1 style="font-size:20pt;">関東で調べるときの入口<small>まず県、次に市区町村。この順で見ると早いです</small></h1>

  <div class="box accentbox">
    <h3>全国どこでも使える入口（住宅まわり）</h3>
    <b>住宅リフォーム推進協議会「地方公共団体における住宅リフォーム支援制度検索サイト」</b><br>
    <span class="note">j-reform.com/reform-support　都道府県 → 市区町村の順に絞り込めます。<br>
    耐震化・バリアフリー・省エネ・環境・防災・同居対応などの分類で探せます。
    <b>終了した制度も掲載されていることがあるため、最新情報は必ず自治体に確認してください。</b></span>
  </div>

  <table style="margin-top:4mm;">
    <tr class="hl"><th style="width:26mm;">都県</th><th>この地域で目立つもの（目安）</th>
      <th style="width:44mm;">主な入口</th></tr>
    {krows}
  </table>

  <div class="box">
    <h3>関東以外の方へ</h3>
    やることは同じです。上の検索サイトで<b>自分の都道府県 → 市区町村</b>と絞り込み、
    そのあと<b>「市区町村名 補助金」</b>でGoogle検索。この2手で、住んでいる自治体の制度はほぼ拾えます。
  </div>

  <div class="box">
    <h3>最後に</h3>
    このシートに載せているのは<b>制度の「型」と探し方</b>です。金額・条件・期限は自治体ごとに違い、毎年変わります。
    <b>必ず自治体の最新の情報で確認してください。</b>このシートは、確認しにいくきっかけを作るためのものです。
  </div>'''

    return [page(p1, "補助金もらい忘れリスト", "1 / 4　使い方と調べ方"),
            page(p2, "補助金もらい忘れリスト", "2 / 4　よくある補助金の型"),
            page(p3, "補助金もらい忘れリスト", "3 / 4　調べた結果を書く"),
            page(p4, "補助金もらい忘れリスト", "4 / 4　関東の入口・全国の探し方")]

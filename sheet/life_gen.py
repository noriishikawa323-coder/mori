...
def hojo():
  ...
  <div class="box" style="margin-top:2.5mm;padding:2.5mm 4.5mm;line-height:1.6;">
    ...
    # 実例（実際に調べたもの）
    ex_helmet = [
        ("江戸川区", "自転車ヘルメット購入補助", "1個あたり最大2,000円（2,000円未満の商品はその価格まで）"),
        ("練馬区", "自転車ヘルメット購入費助成", "1個につき2,000円（販売価格が2,000円未満ならその額）"),
        ("足立区", "自転車用ヘルメットの割引", "対象ヘルメットを3,000円引きで購入できる方式"),
    ]
    ex_env = [
        ("葛飾区", "生ごみ処理機等購入費補助金", "購入価格の1/2、上限20,000円"),
        ("杉並区", "家庭用生ごみ処理機の購入費補助", "購入価格の1/2、上限20,000円"),
        ("品川区", "家庭用生ごみ処理機購入費助成", "申請件数に上限があり、達すると受付終了"),
        ("大田区", "雨水タンク設置助成", "小型タンクは費用の2/3・1基上限40,000円、2基まで（個人）"),
        ("世田谷区", "雨水タンク設置助成", "1基あたり上限35,000円（設置費は上限5,000円）"),
        ("練馬区", "雨水タンク設置助成", "購入価格の1/2、上限25,000円"),
    ]

    def exrow(a, b, c):
        return (f'<tr><td style="width:24mm;height:9mm;font-size:9.4pt;text-align:center;">'
                f'<b>{a}</b></td><td style="width:56mm;font-size:9.2pt;">{b}</td>'
                f'<td style="font-size:9.2pt;">{c}</td></tr>')

    t_helmet = "".join(exrow(*r) for r in ex_helmet)
    t_env = "".join(exrow(*r) for r in ex_env)

    p4 = f'''<h1 style="font-size:20pt;">実際にある補助金の例<small>東京23区で確認できたもの。金額は変わるので、必ず区に確認してください</small></h1>

  <div class="box accentbox">
    <h3>この例は「あることの証明」として載せています</h3>
    <b>金額・条件・受付期間は年度ごとに変わり、終了することもあります。</b>
    実際、<b>世田谷区と大田区の自転車ヘルメット補助は終了しています。</b>
    それでも他の区では続いている。<b>「自分の区にもあるかもしれない」と考える材料にしてください。</b>
  </div>

  <table style="margin-top:3.5mm;">
    <tr class="hl"><th style="width:24mm;">自治体</th><th style="width:56mm;">制度</th>
      <th>内容（確認時点）</th></tr>
    <tr><td colspan="3" style="background:rgba(240,162,2,.18);font-weight:700;
      font-size:9.4pt;height:5.6mm;">自転車ヘルメット</td></tr>
    {t_helmet}
    <tr><td colspan="3" style="background:rgba(240,162,2,.18);font-weight:700;
      font-size:9.4pt;height:5.6mm;">生ごみ処理機・雨水タンク</td></tr>
    {t_env}
  </table>

  <div class="box" style="margin-top:3mm;padding:2.5mm 4.5mm;line-height:1.6;">
    <h3 style="font-size:10.5pt;">この表から分かること</h3>
    <b>1. 同じ制度でも、区によって金額が違います。</b>雨水タンクは25,000円〜40,000円と幅があります。<br>
    <b>2. 隣の区にあって、自分の区にないこともあります。</b>だから自分で確認するしかありません。<br>
    <b>3. 申請件数に上限がある制度があります。</b>品川区の生ごみ処理機は、上限に達すると受付終了です。<br>
